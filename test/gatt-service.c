...
static gboolean desc_get_perms(const GDBusPropertyTable *property,
					DBusMessageIter *iter, void *data)
{
	return TRUE;
}

static gboolean desc_exist_perms(const GDBusPropertyTable *property,
								void *data)
{
	return FALSE;
}
...
static bool populate_descriptor(DBusConnection *conn, const char *uuid,
						const char *chr_path)
{
	static char desc_path[64];
	struct descriptor *desc;
	static int id = 1;

	snprintf(desc_path, sizeof(desc_path), "%s" DESCRIPTOR_PATH, chr_path,
									id++);
	...
	if (g_dbus_register_interface(conn, desc_path,
					...
					desc, NULL) == FALSE) {
		fprintf(stderr, "Couldn't register descriptor interface\n");
		return false;
	}
...
static bool populate_service(DBusConnection *conn, const char *uuid, char *path)
{
	struct service *service;
	static int id = 1;

	snprintf(path, OBJECT_PATH_MAX, SERVICE_PATH, id++);

	service = g_new0(struct service, 1);

	service->uuid = g_strdup(uuid);

	if (g_dbus_register_interface(conn, path, SERVICE_INTERFACE,
					...
					char *path)
{
	...
	snprintf(path, OBJECT_PATH_MAX, "%s" CHARACTERISTIC_PATH,
							service_path, id++);
...
{
	char service_path[OBJECT_PATH_MAX], chr_path[OBJECT_PATH_MAX];
	...
	/*** This descriptor is for tests only ***/
	if (!populate_descriptor(conn, CCC_UUID16, chr_path))
		return;
